...
const CFD_OFFTARGET_PAMS = "NGG / NAG";
const CFD_PROTOSPACER_PAM = "NGG";
const CFD_MAX_MM = 6;
const CFD_THRESHOLD = 0.2;

const downloadOffTargetsCsv = (projName, entries) => {
    const escape = (v) => {
        const s = String(v);
        return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s;
    };
    const meta = [
        `# OptiPrime off-target evaluation`,
        `# project: ${projName || "(unnamed)"}`,
        `# protospacer_pam: ${CFD_PROTOSPACER_PAM} (candidates evaluated from the edit's proximity window)`,
        `# offtarget_pams: ${CFD_OFFTARGET_PAMS} (PAM sites searched genome-wide for each guide)`,
        `# max_mismatches: ${CFD_MAX_MM}`,
        `# cfd_threshold: ${CFD_THRESHOLD}`,
        `#`,
    ].join("\n");
    const header = "proto_id,guide,chr,pos,strand,sequence,pam,cfd_score";
    const rows = entries.flatMap(({ protoId, guide, hits }) =>
        hits.map(h =>
            [protoId, guide, h.chr, h.pos, h.strand, h.sequence, h.pam, h.cfd_score]
                .map(escape).join(",")
        )
    );
    ...
    a.href = url;
    a.download = `offtargets_${(projName || "unnamed").replace(/[^A-Za-z0-9_-]+/g, "_")}.csv`;
...
const Human = ({ state, setState, pushInfo, popInfo, pushError, popError }) => {
    ...
    const runVariantPipeline = (initialPromise) => {
        initialPromise
        .then(async x => {
        ...
        .then(async x => {
            if (x.refSeq) return x;
            pushInfo("human", "Fetching CDS annotations from RefSeq...")
            const refSeq = await coordsToRefSeq(x.coords);
            return { ...x, refSeq };
        })
        .then(x => {
            popInfo("human");
            ...
            const computedPos = computeProteinPos(refSeq, coords.pos);
            const cvAaMatch = alleles.aminoAcidChange?.match(/[A-Za-z*]+(\d+)/);
            const claimedPos = cvAaMatch ? parseInt(cvAaMatch[1]) : null;
            if (computedPos && claimedPos && Math.abs(computedPos - claimedPos) > 5) {
                pushInfo("human",
                    `Note: ClinVar reports this variant at protein position ${claimedPos}, ` +
                    `but the RefSeq transcript places it at position ${computedPos}. ` +
                    `This may indicate signal peptide-based renumbering or a transcript version mismatch. ` +
                    `The genomic coordinate fetched is correct.`
                );
            }
            const { ref, mut } = alleles;
            const uLen = mut.length;
            const eLen = ref.length;
            const refCdsList = getContextExonTranslations(refSeq, coords.pos, _CONTEXT_LEN);
            const mutCdsList = refCdsList.map(cds => updateCDS(cds,
                                                               { start: _CONTEXT_LEN,
    ...
    const submitCvID = (cvID) => {
        ...
        runVariantPipeline(cvIDtoHg38Coords(cvID));
    };

    const submitHgvs = (input) => {
        pushInfo("human", "Parsing HGVS notation...");
        runVariantPipeline(hgvsToHg38Coords(input, assembly || "hg38"));
    };
    ...
    return (
        <Card column="2">
            ...
            <Flex justifyContent="flex-start" alignItems="baseline" padding="5px">
                ...
                <Button children="ClinVar search"
                        onClick={_ => {submitCvID(cvData.item_code);}}
                        disabled={!/\d+/.test(cvData.item_code)}
                        height="42px" />
            </Flex>
            <Divider label="or" margin="10px" />
            <Heading children="Enter HGVS c. notation:" />
            <Flex justifyContent="flex-start" alignItems="baseline" padding="5px">
                <TextField label="hgvs" labelHidden={true}
                           placeholder="e.g., BRCA1 c.5096G>A  or  NM_007294.4:c.5096_5098del"
                           value={hgvs}
                           onChange={e => setHgvs(e.target.value)}
                           onKeyDown={e => {if (e.key === "Enter") {submitHgvs(hgvs);}}}
                           width="400px" />
                <Button children="Parse HGVS"
                        onClick={_ => submitHgvs(hgvs)}
                        disabled={!/c\./i.test(hgvs)}
                        height="42px" />
            </Flex>
            <Divider label="or" margin="10px" />
...
const Unedited = ({ state, setState, pushError, popError }) => {
    ...
    return (
        <Card columnStart="1" columnEnd="-1">
            ...
            <TextField label={"name"} labelHidden={true}
                       placeholder={"Unedited sequence name"}
...
const Edited = ({ state, setState, pushError, popError }) => {
    ...
    return (
        <Card columnStart="1" columnEnd="-1">
            ...
            <TextField label={"name"} labelHidden={true}
                       placeholder={"Edited sequence name"}
...
const PreviewPage = ({ state, setState, onBack, updateTokens }) => {
    ...
    // CFD off-target scoring per guide.
    //   key:   20-nt guide string
    //   value: { status: "loading" | "complete" | "error", hits?, n_candidates?, error? }
    // Only completed entries are persisted to localStorage; loading/error are in-memory only.
    const [cfdResults, setCfdResults] = useState(() => {
        try { return JSON.parse(localStorage.getItem("cfdResults")) || {}; }
        catch { return {}; }
    });
    const cfdInflightRef = useRef(new Set());
    // Per-protospacer expanded state for the off-target panel + global top-N.
    const [expandedOffTargets, setExpandedOffTargets] = useState({});
    // Per-protospacer top-N (keyed by proto id); raw string so the input can be cleared.
    const [topNByProto, setTopNByProto] = useState({});
    // Whole off-target section collapse state.
    const [offTargetSectionOpen, setOffTargetSectionOpen] = useState(false);
    ...
    const [viewAdv, setViewAdv] = useState(false);  // Whether to show the advanced settings
    const [settings, setSettings] = useState({ cell_type: "HeLa" });  // Advanced settings
    // Ref to unedited
    const ref = useRef(null);
    ...
    // CFD OFF-TARGET SCORING for NGG protospacers in proximity to the edit.
    // Proximity window: [editStart - 10, editEnd + 10] in unedited coords.
    // Fires fetchCfdScore (POST -> poll -> GET) per matching guide, caches
    // completed results in localStorage.
    useEffect(() => {
        if (!uSeq || !eSeq || protos.length === 0) return;
        const { minU, preLen } = minEdit(uSeq, eSeq);
        if (minU === undefined || preLen === undefined) return;
        const winStart = preLen - 10;
        const winEnd = preLen + minU.length + 10;
        const proximity = protos.filter(p =>
            p.pamVar === "SpNGG"
            && p.start20 < winEnd
            && p.end20 > winStart
        );
        for (const p of proximity) {
            const guide = p.proto30.slice(4, 24);
            // skip if already scored or in-flight
            if (cfdResults[guide]?.status === "complete") continue;
            if (cfdResults[guide]?.status === "error") continue;
            if (cfdInflightRef.current.has(guide)) continue;
            cfdInflightRef.current.add(guide);
            setCfdResults(prev => ({ ...prev, [guide]: { status: "loading" } }));
            fetchCfdScore(guide)
                .then(result => setCfdResults(prev => {
                    ...
                    const persisted = Object.fromEntries(
                        Object.entries(next).filter(([_, v]) => v.status === "complete")
                    );
                    localStorage.setItem("cfdResults", JSON.stringify(persisted));
                    return next;
                }))
                .catch(err => {
                    console.error("CFD scoring failed for", guide, err);
                    setCfdResults(prev => ({ ...prev, [guide]: {
                        status: "error",
                        error: String(err),
                    }}));
                })
                .finally(() => cfdInflightRef.current.delete(guide));
        }
    }, [protos, uSeq, eSeq, cfdResults]);
    ...
    return (
        ...
        >
            ...
            <Card columnStart="1" columnEnd="-1" padding="0">
                <Heading level={4} children="Select the protospacers you want to evaluate:" />
            </Card>
            <Card columnStart="1" columnEnd="-1" ref={ref} padding="5px">
            ...
            <Card columnStart="1" columnEnd="-1">
                {(() => {
                    ...
                    if (uSeq && eSeq) {
                        const { minU, preLen } = minEdit(uSeq, eSeq);
                        if (minU !== undefined && preLen !== undefined) {
                            const winStart = preLen - 10;
                            const winEnd = preLen + minU.length + 10;
                            downloadable = protos
                                .filter(p => p.pamVar === "SpNGG"
                                          && p.start20 < winEnd
                                          && p.end20 > winStart)
                                .map(p => {
                                    const guide = p.proto30.slice(4, 24);
                                    const r = cfdResults[guide];
                                    if (r?.status !== "complete") return null;
                                    const hits = (r.hits ?? [])
                                        .filter(h => h.sequence.toUpperCase() !== guide.toUpperCase())
                                        .sort((a, b) => b.cfd_score - a.cfd_score);
                                    return hits.length > 0
                                        ? { protoId: p.id, guide, hits }
                                        : null;
                                })
                                .filter(Boolean);
                        }
                    ...
                    return (
                        <Flex justifyContent="space-between" alignItems="center">
                            <Heading level={4}>Off-target evaluation</Heading>
                            <Flex alignItems="center" gap="0.5rem">
                                {downloadable.length > 0 && (
                                    <Button size="small" variation="link"
                                            onClick={() => downloadOffTargetsCsv(state.projName, downloadable)}>
                                        ⤓ download all (CSV)
                                    </Button>
                                )}
                                <Button size="small" variation="link"
                                        onClick={() => setOffTargetSectionOpen(o => !o)}>
                                    {offTargetSectionOpen ? "▼ hide section" : "▶ show section"}
                                </Button>
                            </Flex>
                        </Flex>
                    );
                })()}
                {offTargetSectionOpen && (() => {
                    if (!uSeq || !eSeq) return null;
                    const { minU, preLen } = minEdit(uSeq, eSeq);
                    if (minU === undefined || preLen === undefined) return null;
                    const winStart = preLen - 10;
                    const winEnd = preLen + minU.length + 10;
                    const proximity = protos.filter(p =>
                        p.pamVar === "SpNGG" && p.start20 < winEnd && p.end20 > winStart
                    );
                    if (proximity.length === 0) {
                        return <Text>No NGG protospacers in proximity to the edit.</Text>;
                    }
                    return proximity.map(p => {
                        const guide = p.proto30.slice(4, 24);
                        const r = cfdResults[guide];
                        const isExpanded = !!expandedOffTargets[p.id];
                        const toggle = () => setExpandedOffTargets(prev => ({
                            ...prev, [p.id]: !prev[p.id]
                        }));
                        // Exclude on-target perfect-match (sequence == guide, CFD=1.0).
                        const offTargetHits = (r?.hits ?? []).filter(
                            h => h.sequence.toUpperCase() !== guide.toUpperCase()
                        );
                        const topNStr = topNByProto[p.id] ?? "5";
                        const parsedTopN = parseInt(topNStr, 10);
                        const showN = isNaN(parsedTopN) || parsedTopN < 0
                            ? 0
                            : Math.min(parsedTopN, offTargetHits.length);
                        // Sort by CFD score descending (worst off-targets first); the
                        // API does not guarantee any order.
                        const sortedHits = [...offTargetHits].sort(
                            (a, b) => b.cfd_score - a.cfd_score
                        );
                        return (
                            <Card key={p.id} variation="outlined" marginBlock="0.5rem">
                                <Flex justifyContent="space-between" alignItems="center" wrap="wrap" gap="0.75rem">
                                    <Flex direction="column" gap="0.1rem">
                                        <Text fontWeight="bold">{p.id}</Text>
                                        <Text fontFamily="monospace" fontSize="0.85rem">{guide}</Text>
                                    </Flex>
                                    {(!r || r.status === "loading") && (
                                        <Text fontStyle="italic">computing off-targets… (first request may take ~90 s)</Text>
                                    )}
                                    {r?.status === "error" && (
                                    ...
                                    {r?.status === "complete" && (
                                        <Flex alignItems="center" gap="0.75rem">
                                            <Text>
                                                <b>{offTargetHits.length}</b> off-targets with CFD ≥ 0.2
                                            </Text>
                                            {offTargetHits.length > 0 && (
                                                <Button size="small" variation="link" onClick={toggle}>
                                                    {isExpanded ? "▼ hide" : "▶ show top hits"}
                                                </Button>
                                            )}
                                        </Flex>
                                    )}
                                </Flex>
                                {r?.status === "complete" && isExpanded && offTargetHits.length > 0 && (
                                    <>
                                        <Flex alignItems="center" gap="0.5rem" marginTop="0.5rem">
                                            <Text>Showing top</Text>
                                            <TextField
                                                type="number"
                                                size="small"
                                                width="5rem"
                                                ...
                                                label="top N"
                                                value={topNStr}
                                                onChange={(e) => setTopNByProto(prev => ({
                                                    ...prev, [p.id]: e.target.value
                                                }))}
                                            />
                                            <Text>of {offTargetHits.length} hits</Text>
                                        </Flex>
                                        <Table size="small">
                                            <TableHead>
                                                <TableRow>
                                                    <TableCell as="th">Locus</TableCell>
                                                    <TableCell as="th">Strand</TableCell>
                                                    <TableCell as="th">Sequence</TableCell>
                                                    <TableCell as="th">PAM</TableCell>
                                                    <TableCell as="th">CFD</TableCell>
                                                    <TableCell as="th">UCSC</TableCell>
                                                </TableRow>
                                            </TableHead>
                                            <TableBody>
                                                {sortedHits.slice(0, showN).map((h, i) => (
                                                    <TableRow key={i}>
                                                        <TableCell>{h.chr}:{h.pos}</TableCell>
                                                        <TableCell>{h.strand}</TableCell>
                                                        <TableCell><Text fontFamily="monospace">{h.sequence}</Text></TableCell>
                                                        <TableCell>{h.pam}</TableCell>
                                                        <TableCell>{h.cfd_score.toFixed(3)}</TableCell>
                                                        <TableCell>
                                                            <a
                                                                href={`https://genome.ucsc.edu/cgi-bin/hgTracks?db=hg38&position=${h.chr}:${Math.max(1, h.pos - 50)}-${h.pos + 70}`}
                                                                target="_blank"
            ...
            <Card columnStart="1" columnEnd="-1">
                <SwitchField
                    label="View advanced job settings"
                    labelPosition="start"
                    isChecked={viewAdv}
                    onChange={(e) => { setViewAdv(e.target.checked); }}
                />
            </Card>
            <Card column="2">
                {viewAdv &&
                <>
                    <SelectField
                        label="Cell-type"
                        labelPosition="start"
                        value={settings.cell_type}
                        onChange={(e) => { setSettings(old => ({...old, cell_type: e.target.value})) }}
                        width="50%"
                    >
                        <option value="HeLa">HeLa (default; MMR-competent)</option>
                        <option value="HEK293T">HEK293T (MMR-deficient)</option>
                    </SelectField>
                    <SwitchField
                        label="Synthetic pegRNA"
                        labelPosition="start"
                        onChange={(e) => { setSettings(old => ({...old, pol3: !e.target.checked})) }}
                    />
                </>
                }
            </Card>
...
const Design = ({ updateTokens }) => {
    ...
    useEffect(() => {
        const upperState = {
            ...state,
            uneditedData: { ...state.uneditedData, seq: state.uneditedData.seq.toUpperCase() },
            editedData:   { ...state.editedData,   seq: state.editedData.seq.toUpperCase() }
        };
